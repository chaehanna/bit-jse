...
public class Ex12GradeBook03 {
    ...
    public static void printGrade(int id, String name, int korean, int english, int math) {
        if(korean == -1) {
    ...
    // call by value: 변수를 넘겨주더라도 값만 넘어가지 변수 그 자체는 넘어가지 않음
    // call by reference: 주소값을 넘겨주기 때문에 변경을 가하면 변수에도 변경이 가해짐
    
    // call by value의 경우, 해당 변수의 "값 복사본"이 넘어가고, 따라서 우리가 파라미터에
    // 값을 변하시키더라도 원본 변수의 값은 변하지 않는다.
